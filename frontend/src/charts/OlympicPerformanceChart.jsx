...
const OlympicPerformanceChart = () => {
  ...
  useEffect(() => {
    if (medalsData) {
      const years = [...new Set(medalsData.map(item => item.Year))].sort((a, b) => b - a);
  ...
  useEffect(() => {
    if (medalsData && selectedYear) {
      ...
      if (selectedCountries.length > 0) {
        formatted = formatted.filter(item => selectedCountries.includes(item.country));
      }
  ...
  const availableYears = [...new Set(medalsData.map(item => item.Year))].sort((a, b) => b - a);
  const allCountries = [...new Set(medalsData.filter(item => item.Year === selectedYear).map(item => item.Country))].sort();
  ...
  const handleOpenModal = () => {
    setModalSelectedCountries([...selectedCountries]); // Store the current selected countries in modal
    setIsModalOpen(true);
  };
  ...
  return (
    <div className="min-w-7xl mx-auto p-6 bg-neutral-900 text-white rounded-lg shadow-xl">
      <div className="flex flex-col md:flex-row md:items-end md:justify-between gap-6 mb-6">
        ...
        <button
          ...
          className="bg-yellow-600 hover:bg-yellow-500 text-neutral-900 px-4 py-2 rounded-full"
        >
      ...
      <div className="h-[600px]">
        <ResponsiveContainer width="100%" height="100%">
          ...
          >
            ...
            />
            <YAxis tick={{ fill: "#ccc" }}>
              <Label value="Medals Count" angle={-90} position="left" fill="#ccc" fontSize={14} />
            </YAxis>
            <Tooltip
              ...
              formatter={(value, name, props) => {
                const percentage = ((value / props.payload.total) * 100).toFixed(2);
            ...
            <Bar
              dataKey="gold"
              stackId="a"
              fill={selectedCountries.length > 0 ? "#FF6F61" : "#FF7F50"}
              animationDuration={500}
            />
            <Bar
              dataKey="silver"
              stackId="a"
              fill={selectedCountries.length > 0 ? "#4B9CD3" : "#4682B4"}
              animationDuration={500}
            />
            <Bar
              dataKey="bronze"
              stackId="a"
              fill={selectedCountries.length > 0 ? "#CD7F32" : "#DAA520"}
              animationDuration={500}
            />
          </BarChart>
        </ResponsiveContainer>
      ...
      {isModalOpen && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-start p-6 z-50">
          <div className="bg-gray-800 p-6 rounded-lg w-full max-w-3xl animate__animated animate__fadeIn">
            <div className="flex justify-between items-center mb-4">
              <span className="text-xl font-semibold text-white">Filter Countries ({modalSelectedCountries.length} selected)</span>
              <button
            ...
            <div className="mb-4">
              <input
                ...
                className="px-2 py-1 rounded bg-gray-700 text-white border border-gray-600 text-sm mb-3 w-full"
                value={searchCountry}
                onChange={e => setSearchCountry(e.target.value)}
              />
              <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-2 text-sm">
                {filteredCountries.map(country => (
                  <label key={country} className="flex items-center gap-2 hover:bg-gray-700 rounded-md">
                    <input
                      type="checkbox"
                      value={country}
